...
local function on_attach(client, bufnr)
  ...
  if client:supports_method("textDocument/documentColor") then
    keymap(
      ...
      "[G]oto [C]olor Presentation",
      { "n", "x" }
    )
  end

  if client:supports_method("textDocument/references") then
    keymap("grr", FzfLua.lsp_references, "[G]oto [R]eferences")
  end

  if client:supports_method("textDocument/typeDefinition") then
    keymap("grt", FzfLua.lsp_typedefs, "[G]oto [T]ype Definition")
  end

  if client:supports_method("textDocument/documentSymbol") then
    keymap("gO", FzfLua.lsp_document_symbols, "Document symbols")
    keymap("gW", FzfLua.lsp_live_workspace_symbols, "Workspace symbols")
  end
  ...
  if client:supports_method("textDocument/documentHighlight") then
    local under_cursor_highlights_group =
      vim.api.nvim_create_augroup("rael346/cursor_highlights", { clear = false })
  ...
  if client:supports_method("textDocument/inlayHint") then
    local inlay_hints_group =
      vim.api.nvim_create_augroup("rael346/toggle_inlay_hints", { clear = false })
  ...
  --       },
  --     }, nil, bufnr)
  --   end, string.format("Fix all %s errors", client.name == "eslint" and "ESLint" or "Stylelint"))
  -- end
end
