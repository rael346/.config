...
vim.keymap.set(
  "n",
  "<leader>sm",
  function() FzfLua.marks({ marks = "%l" }) end,
  { desc = "[S]earch [M]arks" }
)
...
vim.keymap.set(
  "n",
  "<leader>sn",
  function() FzfLua.files({ cwd = vim.fn.stdpath("config") }) end,
  { desc = "[S]earch [N]eovim files" }
)

vim.keymap.set("n", "<leader>sb", FzfLua.dap_breakpoints, { desc = "[S]earch [B]reakpoints" })
